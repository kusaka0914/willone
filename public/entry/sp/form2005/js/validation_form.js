...
$.validationMessages = {
    ...
    'retirement_intention'       : {'id': 'retirement_intention_errmsg', 'message': 'お仕事のご状況を選択して下さい'},
    ...
    'terms_checkbox'             : {'id': 'terms_checkbox_errmsg',       'message': '利用規約を確認してください'},
};
$.valid_mail_result = "";
...
$.validate_form = function (pagenum) {
    ...
    if (pagenum == 1) {
        ...
        if (Object.keys(result.errors).length == 0) {
            result = $.validate_item('retirement_intention');
            if (Object.keys(result.errors).length == 0) {
                result = $.validate_item('mob_mail');
                if (Object.keys(result.errors).length == 0) {
                  result = $.validate_item('terms_checkbox');
              }
            }
        }
    }
...
$.errmsg_on = function (target) {
    ...
    // セレクタを $('.bx-viewport') に変更。 JINZAI-22325
...
$.validate_item = function (input_id) {
    ...
    switch (input_id) {
        ...
        case 'moving_flg':
          ...
          case 'terms_checkbox':
              const termsCheckbox = $('#' + input_id).prop("checked");
              if (termsCheckbox == 1) {
                $.errmsg_off(input_id);
              } else {
                result = false;
