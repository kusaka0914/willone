...
$(function() {
  ...
  $('input[name="license[]"]').on("change", function() {
    $('input[name="license[]"] + label').addClass('btn_off');
    $('#slide_student_btn').addClass('btn_off');
    $('input[name="license[]"]:checked + label').removeClass('btn_off');
  ...
  $("#slide_student_btn").on("click", function() {
    ...
    if ($('.office-name').length > 0) {
      // .officenameが存在する場合の処理
      $('.bx-viewport').css('min-height', '850');
      $('html, body').animate({scrollTop: 200}, 300);
    } else {
      // .officenameが存在しない場合の処理
      $('.bx-viewport').css('min-height', '700');
    }
    $(".hands").hide();
  ...
  $('input[name="req_emp_type[]"]').on("change", function() {
    var result = $.validate_form(pageNo);
    $('input[name="req_emp_type[]"] + label').addClass('btn_off');
    $('input[name="req_emp_type[]"]:checked + label').removeClass('btn_off');
    // 手の位置を調整
    var isNext = $.handsSetting(pageNo, result);
    if ($(this).prop('checked')) {
      $('#req_date_errmsg').hide();
          var step3Btn = $("#req_emp_types_area").offset().top + 200;
          $('html, body').animate({scrollTop: step3Btn}, 300);
    }
  ...
  $('input[name="req_date"]').on("change", function() {
    var result = $.validate_form(pageNo);
    $('input[name="req_date"] + label').addClass('btn_off');
    $('input[name="req_date"]:checked + label').removeClass('btn_off');
  ...
  // お名前（漢字）
  $("#name_kan").on("keydown keyup keypress change input", function(event) {
    if (event.key === 'Enter') {
      event.preventDefault(); // 実機のエンターを押すとsubmitされてしまうため、Enterキーのデフォルト動作を無効化
  ...
  $("#mob_phone").on("change", function() {
    ...
    // 利用規約
    $('input[name="termsCheckbox"]').on("change", function() {
      var result = $.validate_form(pageNo);
      // 手の位置を調整
      var isNext = $.handsSetting(pageNo, result);
    ...
    // 利用規約、チェックボックス
    $('input[name="terms_checkbox"]').on("change", function() {
      var result = $.validate_form(pageNo);
      // 手の位置を調整
      var isNext = $.handsSetting(pageNo, result);
  ...
  $(".hands").animate({ top: position.top + 70 }, 0);
});
...
$.onSlideAfterHandler = function($slideElement, oldIndex, newIndex) {
  ...
  if (pagenum < 6) {
    ...
    $("img#form_status")
        ...
        .attr('src', $imgPath + '/status/' + pagenum + '.png' );
    ...
    if (pagenum < 5) {
      ...
      $("a.bx-next").html(
        '<span class="lastBtn">応募エントリー</span>'
      );
    }
    ...
    switch (pagenum) {
      ...
      case 2:
        ...
        $(".terms-link, .cover").hide();
        if ($('.office-name').length > 0) {
          $('.bx-viewport').css('min-height', 965);
        } else {
          $('.bx-viewport').css('min-height', 850);
        }
        break;
      ...
      case 4:
        ...
        if ($('.office-name').length > 0) {
          $('.bx-viewport').css('min-height', 640);
        } else {
          $('.bx-viewport').css('min-height', 470);
        }
      ...
      case 5:
        ...
        $(".cover, #req_date_area").hide();
        if ($('.office-name').length > 0) {
          $('.bx-viewport').css('min-height', 1000);
        } else {
          $('.bx-viewport').css('min-height', 910);
        }
  ...
  switch (pagenum) {
    case 1:
      if (Object.keys(result.errors).length == 0) {
        ...
        // ステップ１
        // office-nameがあり、有資格者であればスクロール
        setTimeout(function() {
          if ($('.office-name').length > 0) {
            if ($('#graduation_year_area').is(':hidden')) {
        ...
        // 保有資格
        var position = $("#license_area").offset();
        $(".hands").animate({ top: position.top + 60 }, 0);
    ...
    case 4:
      if (Object.keys(result.errors).length == 0) {
        ...
        // かな
        // 非表示(display:none)の場合、offsetが0になるので
        // name_kanからのポジションとする
        var position = $("#name_kan").offset();
        $(".hands").animate({ top: position.top + 50 }, 0);
    ...
    case 5:
      if (Object.keys(result.errors).length == 0) {
        ...
        $(".hands").animate({ top: position.top }, 0);
      } else if ("terms_checkbox" in result.errors) {
        var position = $("#mob_phone").offset();
        $(".hands").animate({ top: position.top + 700 }, 0);
      } else {
        var position = $("#mob_phone").offset();
        $(".hands").animate({ top: position.top }, 0);
      }
      const termsCheckbox = $('#terms_checkbox').prop("checked");
      if (isNext === true && termsCheckbox === false) {
        isNext = false;
      }
