...
$(function () {
  ...
  slider = $(".bxslider").bxSlider({
    ...
    onSlideBefore: function($slideElement, oldIndex, newIndex) {
      ...
      // 3番目のスライドから2番目のスライドに戻る場合
      if (oldIndex === 2 && newIndex === 1 && studentBothSelected()) {
        isTransitioning = true; // 遷移中に設定
        // 「常勤」にチェックを外す
        $('#req_emp_type_1').prop('checked', false);
        setTimeout(function() {
          slider.goToSlide(0);
          $('ul.status').attr('data-page-num', 1);
          $(".bx-controls")
            .removeClass("index3")
            .removeClass("index2")
            .addClass("index1");
        ...
        return false; // スライド遷移をキャンセル
      }

      // 最初のスライドでstudentBothSelectedがtrueの場合
      if (oldIndex === 0 && studentBothSelected()) {
        isTransitioning = true; // 遷移中に設定
        // 「常勤」にチェックを入れる
        $("#req_date_5").prop("checked", true);
        // 「いつ頃の求人をお探しですか？」非表示とデフォルト値
        $("#req_date_area").hide();
        $('#req_emp_type_1').prop('checked', true);
        setTimeout(function() {
          slider.goToSlide(2);
    ...
    onSlideAfter: function($slideElement, oldIndex, newIndex) {
      // 遷移中の場合はカスタムハンドラを呼び出さない
      if (isTransitioning) return;

      // スライド完了後のカスタムハンドラを呼び出し
      $.onSlideAfterHandler($slideElement, oldIndex, newIndex);
    },
    onSliderLoad: function() {
      ...
      function fadeIn() {
        $('.step1-2').fadeIn(100);
        $('.bar').css('display', 'grid').fadeIn(100);
        // 表示を戻す
        $("#Step2, #Step3, #Step4, #Step5").show();
      }
  ...
  // デフォルトで職種選択されていたらcoverを表示する
  $('#license_container li').each(function() {
  ...
  $('input[name="license[]"]').on("change", function () {
    ...
    // 資格の選択が全解除されてもアラート文言を表示しない処理
    const cnt = $('input[name="license[]"]:checkbox:checked').length;
    if(cnt == 0){
      return false;
    }

    if ($("#slide_student_btn").is(":hidden")) {
      $(".bxslider > li").eq('0').height("auto");
    }
  ...
  $("#slide_student_btn").on("click", function () {
    $(".bxslider > li").eq('0').height("auto");
    $(".hands").animate({ top: "462" }, 300);
    $(".formItemWorker").slideToggle("normal");
    $("#graduation_year_area").slideToggle("normal");
  });
...
$.onSlideAfterHandler = function ($slideElement, oldIndex, newIndex) {
  ...
  if (pagenum == 5) {
    $("#retirement_intention_7").prop("checked", true);
    $("#retirement_intention_area").hide();
    // 規約リンクの表示
...
$.setNextBtn = function(isNext) {
    $('.arrow_step5').hide();
    if (isNext) {
      var step2Btn = $("#footer").offset().top;
      $("html, body").animate({ scrollTop: step2Btn }, "slow");
        // ON
        //$('.btn-area > a').removeClass('off');
        if (pageNo !== 2) {
          $('.cover').fadeIn();
...
// 両方が選択されているかをチェックする関数
function studentBothSelected() {
  var licenseSelected = $('input[name="license[]"]:checked').length > 0;
  var graduationYearSelected = $("#graduation_year").val() !== "";
  return licenseSelected && graduationYearSelected;
}
