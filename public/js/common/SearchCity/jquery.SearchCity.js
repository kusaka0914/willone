jQuery(document).ready(function($) {
     ...
     * 転居の検討の表示切替
     */
    function switchDisplayMoving()
    {
        if ($("#moving_flg_area").length > 0 && $("#not_move").length > 0) {
            var prefId = $(SearchCity.dst_pref).val();
            var prefList = $("#not_move").val().split(',');
            // リストに存在しない場合は転居の検討を問う（表示させる）
            if (prefId != "" && prefList.indexOf(prefId) == -1) {
                $("#moving_flg_area").show();
            } else {
                $("#moving_flg_area").hide();
            }
        }
    }
});