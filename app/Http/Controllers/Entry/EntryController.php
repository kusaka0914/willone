...
class EntryController extends Controller
{
    ...
    {
        $t = $allRequest['t'] ?? null;
        if ($t && !empty($allRequest['req_emp_type'])) {
            $reqEmpType = $this->getSelectDataToText($allRequest['req_emp_type'], 'emp_type');
            $nextYear = Carbon::now()->addYear()->format('Y年卒');
            if (in_array($t, self::KUROHON_LP)) {
                // 翌年卒or不明かつ常勤orこだわらない の場合kurohonfinを表示
                if (in_array($allRequest['graduation_year'], [$nextYear, '不明']) && in_array($reqEmpType, [self::ENTRY_FULLTIME_EVENT, self::ENTRY_ANY_EVENT])) {
                    return "{$this->device}/entry/kurohonfin";
                } else {
                    return "{$this->device}/entry/kurohon_woa_fin";
                }
            } elseif (in_array($t, self::KUROHON_LP_STUDENT)) {
                return "{$this->device}/entry/kurohon_woa_fin";
            }
        }
