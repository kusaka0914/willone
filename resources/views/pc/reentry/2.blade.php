...
<body id="pagetop" class="likeModal pc style-mamiya reclamation reclamation-second">
  <div class="header">
  ...
  <div class="likeModal_contents ani-showFloaty">
    <div id="wrap">
      <!-- 3秒転職アンケート -->
      <div class="">
        <div class="formMuscle_box step1 pc" style="padding-top: 1em;">
             ...
             @include ( 'common/_hidden_reentry' )
              ...
              <div class="swiper-container">
                <div class="swiper-wrapper">
                  <!-- 〓〓〓　スライド　〓〓〓 -->
                  <di class="swiper-slide swiper-no-swiping">
                    <div class="formMuscle_formItem" style="padding:0 4px;">
                      <div class="formMuscle_error" style="display: none;">ご用件を教えてください！</div>
                      <div class="c-form-title-wrap">
                        <span class="form-label form-label-required">必須</span>
                        <p class="c-form-base-title">ご用件を教えてください</p>
                      </div>
                      ...
                      @endif
                      <select name="contact_inquiry" id="" class="reclamation-select">
                        @foreach(config('ini.REENTRY_CONTACT_INQUIRY') as $key => $one)
                        <option value="{{$key}}">
                        {{$one}}
                        </option>
                        @endforeach
  ...
  //      autoHeight: true,
    ...
    // ご用件
    $('.reclamation-select').on('change', function(){
      if(reclamation_select == 0) {
        reclamation_select = 1;
