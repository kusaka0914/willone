...
<head>
	...
	<script type="text/javascript" src="{{addQuery('/woa/js/jquery-1.11.1.min.js')}}"></script>
	<script type="text/javascript" src="{{addQuery('/woa/js/jquery.easing.1.3.js')}}"></script>
	<script type="text/javascript" src="{{addQuery('/woa/js/jquery.autoKana.js')}}"></script>
	<script type="text/javascript" src="{{addQuery('/woa/js/common/ga_common.js')}}"></script>
	<script src="{{addQuery('/woa/js/common/ga4_common.js')}}"></script>
	<script type="text/javascript" src="{{addQuery('/woa/js/common/SearchCity/jquery.SearchCity.js')}}"></script>
	<script type="text/javascript" src="{{addQuery('/woa/entry/sp/form24/js/jq.bxslider_for_horizscrl_form.js')}}"></script>
	<script type="text/javascript" src="{{addQuery('/woa/entry/sp/form24/js/validation_form.js')}}"></script>
	<script src="{{addQuery('/woa/entry/sp/form24/js/horizontal_scroll_form.js')}}"></script>
	<link href="{{addQuery('/woa/entry/sp/form24/css/style.css')}}" rel="stylesheet" type="text/css">
		...
		<div class="modal branch">
			<div class="modalBody intro">
				<div id="branch">
					...
					<a class="close branch_btn-A" onclick="$('#branch_data').val('A');$.sendGA(1, 'STEP1_branchA');"><span>近いうちに転職したい</span></a>
					<a class="close branch_btn-B" onclick="$('#branch_data').val('B');$.sendGA(1, 'STEP1_branchB');"><span>今は情報収集したい</span></a>
				</div>
			</div>
...
@include('common._hidden_entry')
			<div id="dialog_form" data-initialstate="false">
				<div id="dialog_header">
					<h1><img src="/woa/entry/sp/form24/img/head.png" alt="ウィルワン ハローワーク非掲載も多数！新着・会員限定のレア求人をご紹介！"></h1>
@include('common._error_message')
					<div class="status"><img id="form_status" data-page-num="1" src="/woa/entry/sp/form24/img/status/1.png"></div>
				</div>
						...
						<li>
							<div id="Step1" style="visibility: visible;">
									...
									@foreach($licenseList as $value)
										...
										<li class="col @if(@in_array($value->id, $license)) checked @endif">
											...
											<label for="license_{{$value->id}}" class="checkbox">
												<img src="/woa/entry/sp/form24/img/license_{{$value->id}}.png" width="45px">{{$value->license}}
											</label>
										</li>
										@endif
									@endforeach
                                        <li class="col slide-student-btn" id="slide_student_btn">
											<img src="/woa/entry/sp/form24/img/license_student.png" width="45px">学生の方はこちら
											<img src="/woa/entry/sp/form24/img/352466_arrow_down_icon.svg" width="45px">
                                            <input type='hidden' id='license_student_cnt' value={{$license_student_cnt}}>
                                            <input type='hidden' id='license_next_step_flag' value={{$license_next_step_flag}}>
										</li>
								...
								<div id="graduation_year_area" style="display:@if($student) block; @else none; @endif">
									...
									<div class="error_message" id="graduation_year_errmsg" style="display: none;">{{ $errors->first('graduation_year') }}</div>
									@else
								...
								<div id="req_emp_types_area" class="formItem">
									...
									@foreach ($reqEmpTypeList as $value)
										<li class="col @if(@in_array($value->id, $req_emp_type)) checked @endif">
											<input type="radio" value="{{$value->id}}" id="req_emp_type_{{$loop->iteration}}" name="req_emp_type[]" @if(@in_array($value->id, $req_emp_type)) checked checked="checked" @endif >
											<label for="req_emp_type_{{$loop->iteration}}" class="radio"><img src="/woa/entry/sp/form24/img/req_emp_type_{{$value->id}}.png" width="45px">{!! nl2br($value->emp_type_br) !!}</label>
										</li>
									@endforeach
									</ul>
							...
							<div id="Step2" style="visibility: visible;">
								<div id="req_date_area" class="formItem">
									...
									@foreach ($req_dateList as $value)
											<li class="col @if($value->id == $req_date) checked @endif">
											<input type="radio" value="{{$value->id}}" id="req_date_{{$loop->iteration}}" name="req_date" @if($value->id == $req_date) checked checked="checked" @endif disabled="disabled">
											<label for="req_date_{{$loop->iteration}}" class="radio"><img src="/woa/entry/sp/form24/img/req_date_{{$value->id}}.png" width="45px">{{$value->req_date}}</label>
											</li>
									@endforeach
									</ul><!-- end formItem -->
							...
							<div id="Step3" style="visibility: visible;">
								<div id="zip_area" class="formItem">
                                    ...
                                    <div class="addTxt message">
                                        <img src="/woa/entry/sp/form24/img/ico_01-2.png" alt="ポイント">
...
<script>
	...
	jQuery(function($){
        ...
        // お気持ちを飛ばすパラメーターがある場合飛ばす
        const branch_skip = params.get('branch_skip');
        if (branch_skip == 1) {
            $('.branch_btn-A').trigger("click");
            $('#branch_data').val('');
        }
	});
    function branchOperation(){
        ...
        const license_student_cnt = $('#license_student_cnt').val();
        if (license_student_cnt > 0) {
            $('#slide_student_btn').trigger("click");
        }
        const license_next_step_flag = $('#license_next_step_flag').val();
        if (license_next_step_flag == '1') {
