...
<head>
    ...
    <script type="text/javascript" src="{{addQuery('/woa/entry/sp/form1001/js/jq.bxslider_for_horizscrl_form.js')}}"></script>
    <script type="text/javascript" src="{{addQuery('/woa/entry/sp/form1001/js/validation_form.js')}}"></script>
	<script src="{{addQuery('/woa/entry/sp/form1001/js/horizontal_scroll_form.js')}}"></script>
	<link href="{{addQuery('/woa/entry/sp/form1001/css/style.css')}}" rel="stylesheet" type="text/css">
		...
		<div class="modal branch">
			<div class="modalBody intro">
				<div id="branch">
					...
					<a class="close branch_btn-A" onclick="$('#branch_data').val('A');"><span>近いうちに転職したい</span></a>
					<a class="close branch_btn-B" onclick="$('#branch_data').val('B');"><span>今は情報収集したい</span></a>
					<div class="hands_modal"></div>
				</div>
			</div>
			<div class="modalBK"></div>
		</div>
		<div class="hands" style="right: 8%; bottom: 65%;"></div>
					...
					<ul class="bxslider">
						<li>
							<div id="Step1" style="visibility: visible;">
									...
									<ul class="formItem">
										...
										<li class="col @if(@in_array($value->id, $license)) checked @endif">
											...
											<label for="license_{{$value->id}}" class="checkbox">
												<img src="/woa/entry/sp/form1001/img/license_{{$value->id}}.png" width="45px">{{$value->license}}
											</label>
										</li>
										@endif
										@endforeach
										...
										<li class="col" id="slide_student_btn">
											<img src="/woa/entry/sp/form1001/img/license_student.png" width="45px">学生
										</li>
									</ul>
								...
								<h2>紹介者氏名</h2>
								<input type="text" size="20" name="introduce_name" id="introduce_name" value="{{ old('introduce_name')}}" placeholder="例：整体花子" maxlength="64"/>
								<div class="error_message" id="introduce_name_errmsg" @if($errors->has('introduce_name')) style="display: block;" @endif>{{ $errors->first('introduce_name') }}</div>
							</div>
						</li>{{-- /STEP1 --}}
							...
							<div id="Step2" style="visibility: visible;">
								<div id="req_emp_types_area">
									...
									@foreach ($reqEmpTypeList as $value)
										...
										<label for="req_emp_type_{{$loop->iteration}}" class="radio"><img src="/woa/entry/sp/form1001/img/req_emp_type_{{$value->id}}.png" width="45px">{!! nl2br($value->emp_type_br) !!}</label>
										</li>
									@endforeach
									</ul><!-- end formItem -->
								...
								<div id="req_date_area">
									...
									@foreach ($req_dateList as $value)
											...
											<label for="req_date_{{$loop->iteration}}" class="radio"><img src="/woa/entry/sp/form1001/img/req_date_{{$value->id}}.png" width="45px">{{$value->req_date}}</label>
											</li>
									@endforeach
									</ul><!-- end formItem -->
