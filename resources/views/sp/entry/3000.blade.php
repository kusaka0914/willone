...
<head>
	...
	<link rel="stylesheet" type="text/css" href="{{ addQuery('/woa/entry/sp/form3000/css/style.css') }}">
...
<body class="step1">
		...
		<form id="form" name="form1" action="@if (preg_match('/jinzaibank.com/', $_SERVER['HTTP_HOST'])){{ route('EntryFin')}}@else{{ route('EntryFinFromOld')}}@endif" method="post">
			...
			<div id="dialog_form" data-initialstate="false">
				<div id="dialog_header">
					<h1><img src="/woa/entry/sp/form3000/img/head.png" alt="【国試黒本】柔道整復師・鍼灸師・マッサージ師試験の参考書/国家試験対策"></h1>
									...
									<ul id="license_container" class="colWrap">
										...
										<li class="col @if(@in_array($value->id, $license)) checked @endif">
											...
											<label for="license_{{ $value->id }}" class="checkbox">
												<img src="/woa/entry/sp/form3000/img/license_{{ $value->id }}.png" width="45px">{{ preg_replace("/（学生）/", '', $value->license) }}
											</label>
										</li>
										@endif
										@endforeach
				...
				<input name="req_date" value='3' type="hidden">
								<div id="req_emp_types_area" class="formItem">
									...
									@foreach ($reqEmpTypeList as $value)
										...
										<label for="req_emp_type_{{ $loop->iteration }}" class="radio"><img src="/woa/entry/sp/form3000/img/req_emp_type_{{ $value->id }}.png" width="45px">{!! nl2br($value->emp_type_br) !!}</label>
										</li>
									@endforeach
									</ul>
                  	...
                  	<p>残り&nbsp;<span>50%</span>&nbsp;！</p>&nbsp;カンタン60秒で求人検索！
								...
								<div id="zip_area">
                                    ...
                                    <div class="addTxt message">
                                        <img src="/woa/entry/sp/form3000/img/ico_01-2.png" alt="ポイント">
	...
	<script src="/woa/entry/sp/form3000/js/jq.bxslider_for_horizscrl_form.js?20221121"></script>
	<script src="{{addQuery('/woa/js/common/ga4_common.js')}}"></script>
	<script src="{{ addQuery('/woa/entry/sp/form3000/js/validation_form.js') }}"></script>
	<script src="{{addQuery('/woa/entry/sp/form3000/js/horizontal_scroll_form.js')}}"></script>
	<script src="{{addQuery('/woa/entry/sp/form3000/js/sp_modal_browserback.js')}}"></script>
