...
<head>
	...
	<link rel="stylesheet" type="text/css" href="{{ addQuery('/woa/entry/sp/form20/css/style.css') }}">
...
<body class="step1" style='overflow:hidden;'>
		...
		<form id="form" name="form1" action="@if (preg_match('/jinzaibank.com/', $_SERVER['HTTP_HOST'])){{ route('EntryFin')}}@else{{ route('EntryFinFromOld')}}@endif" method="post">
			...
			<div id="dialog_form" data-initialstate="false">
				<div id="dialog_header">
					<h1><img src="/woa/entry/sp/form20/img/head.png" alt="ウィルワン ハローワーク非掲載も多数！新着・会員限定のレア求人をご紹介！"></h1>
									...
									<ul id="license_container" class="colWrap">
										...
										<li class="col @if(@in_array($value->id, $license)) checked @endif">
											...
											<label for="license_{{ $value->id }}" class="checkbox">
												<img src="/woa/entry/sp/form20/img/license_{{ $value->id }}.png" width="45px">{{ $value->license }}
											</label>
										</li>
										@endif
										@endforeach
										...
										<li class="col" id="slide_student_btn">
											<img src="/woa/entry/sp/form20/img/license_student.png" width="45px">学生
                  	...
                  	<p>残り&nbsp;<span>70%</span>&nbsp;！</p>&nbsp;カンタン60秒で求人検索！
								...
								<div id="req_date_area" class="formItem">
									...
									@foreach ($req_dateList as $value)
											...
											<label for="req_date_{{ $loop->iteration }}" class="radio"><img src="/woa/entry/sp/form20/img/req_date_{{ $value->id }}.png" width="45px">{{ $value->req_date }}</label>
											</li>
									@endforeach
									</ul>
								...
								<div id="req_emp_types_area" class="formItem">
									...
									@foreach ($reqEmpTypeList as $value)
										...
										<label for="req_emp_type_{{ $loop->iteration }}" class="radio"><img src="/woa/entry/sp/form20/img/req_emp_type_{{ $value->id }}.png" width="45px">{!! nl2br($value->emp_type_br) !!}</label>
										</li>
									@endforeach
									</ul>
                  	...
                  	<p>残り&nbsp;<span>50%</span>&nbsp;！</p>&nbsp;カンタン60秒で求人検索！
								...
								<div id="zip_area">
                                    ...
                                    <div class="addTxt message">
                                        <img src="/woa/entry/sp/form20/img/ico_01-2.png" alt="ポイント">
	...
	<script src="/woa/entry/sp/form20/js/jq.bxslider_for_horizscrl_form.js?20221121"></script>
	<script src="/woa/entry/sp/form20/js/validation_form.js"></script>
	<script src="{{addQuery('/woa/entry/sp/form20/js/horizontal_scroll_form.js')}}"></script>
	<script src="/woa/entry/sp/form20/js/sp_modal_browserback.js?20221114"></script>
